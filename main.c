...
#define SSID2     "NETGEAR87"
#define PASSWORD "quickviolet413"

uint8_t RemoteIP[] = {134,29,63,165};
...
const char *getSecurity(int select)
{
	char *type;
...
//		
		if(WIFI_ListAccessPoints(&available, WIFI_MAX_APS) == WIFI_STATUS_OK)
		{
			for(int i = 0; i < available.count; i++)
			{
				//Get the security type in private function
				int type = available.ap[i].Ecn;
				const char *outputEcn = getSecurity(type); //Output "String" for TERMOUT
				
				TERMOUT("SSID: %s\n\r", available.ap[i].SSID);
				//TERMOUT("Security Type: %d \n\r", available.ap[i].Ecn); // Security Type
				TERMOUT("%s\n\r", outputEcn); // Security Type
				TERMOUT("RSSI: %d dBm \n\r", available.ap[i].RSSI); // RSSI
				
				//MAC Address
				TERMOUT("MAC Address: %X:%X:%X:%X:%X:%X\n\r",
								available.ap[i].MAC[0],
								available.ap[i].MAC[1],
								available.ap[i].MAC[2],
								available.ap[i].MAC[3],
								available.ap[i].MAC[4],
								available.ap[i].MAC[5]);
				
				TERMOUT("RF Channel: %d\n\r", available.ap[i].Channel); //RF Channel
				TERMOUT("Freq. Offset: %d\n\n\r", available.ap[i].Offset); //Freq Offset
			}
		}

		
		//WIFI_ListAccessPoints(WIFI_APs_t *APs, uint8_t AP_MaxNbr);
    ...
    if( WIFI_Connect(SSID2, PASSWORD, WIFI_ECN_WPA2_PSK) == WIFI_STATUS_OK)
    {
